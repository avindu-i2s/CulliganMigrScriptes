...
, case  WHEN cs.service_rate  is null then ''
	...
	else cONCAT('10', '-',CS.ACCOUNT_NUMBER,'-',CS.SERVICE_NUMBER)
	end ContractID_SC
, 1 LineNo_PMSC
, 'DAY' StartUnit_PM
...
, 'EDG_TA_DEL' ResourceGroup_PM
,CASE 
WHEN cs.delv_truck_1 IS NOT NULL THEN CONCAT('TA_TRUCK_',cs.delv_truck_1)
...
FROM 
dbo.WF_TA_CustServ cs
INNER JOIN [dbo].[WF_TA_CUSTOMER] C ON cs.ACCOUNT_NUMBER = C.ACCOUNT_NUMBER
inner join dbo.customer_active_scope ac on ac.legacy_number = cs.account_number and ac.legacy_number = c.account_number and ac.legacy_migration_source = '10 - Tallahassee'inner join dbo.WF_TA_ServiceCodes sc on cs.service_c = sc.SERVICE_C
...
left join (select CWG_Part_Number,ifs_number2, IFS_Description from dbo.EDG_NonCullParts_CWG where CWG_Part_Number is not null) X1
 ...
 left join [dbo].[WF_TA_CustOverride] O on cs.ACCOUNT_NUMBER = o.ACCOUNT_NUMBER and cs.TRANSACTION_C = o.TRANSACTION_C
...
left join [dbo].[WF_All_ContractServiceTranslation_P1] ST ON ST.BRANCH = '10 - Tallahassee' AND CS.SERVICE_C = ST.service_code
