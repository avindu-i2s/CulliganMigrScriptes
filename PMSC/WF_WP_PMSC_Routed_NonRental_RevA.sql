...
, case  WHEN cs.service_rate  is null then ''
	...
	else cONCAT('06', '-',CS.ACCOUNT_NUMBER,'-',CS.SERVICE_NUMBER)
	end ContractID_SC
, 1 LineNo_PMSC
, 'DAY' StartUnit_PM
...
, 'EDG_WP_DEL' ResourceGroup_PM
,CASE 
WHEN cs.delv_truck_1 IS NOT NULL THEN CONCAT('WP_TRUCK_',cs.delv_truck_1)
...
FROM 
dbo.WF_WP_CustServ cs
INNER JOIN [dbo].[WF_WP_CUSTOMER] C ON cs.ACCOUNT_NUMBER = C.ACCOUNT_NUMBER
inner join dbo.customer_active_scope ac on ac.legacy_number = cs.account_number and ac.legacy_number = c.account_number and ac.legacy_migration_source = '06 - West Palm Beach'inner join dbo.WF_WP_ServiceCodes sc on cs.service_c = sc.SERVICE_C
...
left join (select CWG_Part_Number,ifs_number2, IFS_Description from dbo.EDG_NonCullParts_CWG where CWG_Part_Number is not null) X1
 ...
 left join [dbo].[WF_WP_CustOverride] O on cs.ACCOUNT_NUMBER = o.ACCOUNT_NUMBER and cs.TRANSACTION_C = o.TRANSACTION_C
...
left join [dbo].[WF_All_ContractServiceTranslation_P1] ST ON ST.BRANCH = '06 - West Palm Beach' AND CS.SERVICE_C = ST.service_code
