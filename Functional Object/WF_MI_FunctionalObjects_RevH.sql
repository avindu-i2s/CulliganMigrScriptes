...
DECLARE
@SITE_ VARCHAR(10) = '74007',
@MigSource_ varchar(50) = '07 - Miami'
SELECT DISTINCT
...
/*, case when ce.DEALER_OWNED_C = 'D' then  ( case 
	when UPPER(ce.SERIAL_NUMBER) LIKE 'ZZ%'  THEN Concat(Concat(z.TranslationListPart,'-'),upper(REPLACE(ce.INSTALLED_DATE,'-','')))
	when UPPER(ce.SERIAL_NUMBER) LIKE 'xxzz%'  THEN Concat(Concat(z.TranslationListPart,'-'),upper(REPLACE(ce.INSTALLED_DATE,'-','')))
	ELSE Concat(Concat(z.TranslationListPart,'-'),upper(ce.SERIAL_NUMBER)) 
	END)
...
inner join dbo.WF_MI_ServiceCodes sc on cs.service_c = sc.SERVICE_C
