...
DECLARE
@SITE_ VARCHAR(10) = '74016',
@MigSource_ varchar(50) = '16 - Kingsland'
...
inner join dbo.customer_active_scope ac on ac.legacy_number = ce.account_number  and ac.legacy_migration_source = @MigSource_
LEFT JOIN WF_KL_EquipmentMaster eq ON ce.SERIAL_NUMBER = eq.SERIAL_NUMBER AND ce.EQUIPMENT_C = eq.EQUIPMENT_C and eq.EQUIPMENT_C = ec.equipment_c and eq.CURRENT_ACCOUNT = ce.ACCOUNT_NUMBER and eq.CURRENT_ACCOUNT = ac.legacy_number
left join [dbo].[WF_EQ_CrossRef] z on ce.equipment_c = z.equipment_c and z.db_source = @MigSource_
left join dbo.IFS_FA_ID FA ON CE.EQUIPMENT_C = FA.EQUIPMENT_C AND FA.ACCOUNT_NUMBER = CE.ACCOUNT_NUMBER AND FA.SERIAL_NUMBER = CE.SERIAL_NUMBER
inner join dbo.IFS_WF_FUNCOBJ fo on fo.branch_datasource =  @MigSource_ and ce.EQUIPMENT_C = fo.equipment_c and CE.ACCOUNT_NUMBER = FO.ACCOUNT_NUMBER AND CE.SERIAL_NUMBER = FO.SERIAL_NUMBER AND AC.customer_no = FO.IFS_cust_no
...
inner join dbo.WF_KL_ServiceCodes sc on cs.service_c = sc.SERVICE_C
...
where cs.SERV_TERM_DATE = '' --and CS.SERVICE_C <> 'HOM'
